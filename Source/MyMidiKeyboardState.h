...
public:
    ...
    */
    bool isKeyDown (int midiChannel, int midiNoteNumber) const noexcept;

    /** Returns true if the given midi key is held down OR sustaining on any of a set of midi channels.
    ...
    */
    ...
    /** Turns a specified note off.
        ...
        SD: added force flag, to schedule MIDI note-off message even for sustaining notes
    */
    void noteOff (int midiChannel, int midiNoteNumber, float velocity, bool force = false);
    ...
    */
    ...
    /** SD added: Sets the sustain-pedal state (down/true = sustaining, up/false = not sustaining)
        for the given midi channel.
    */
    void pedal(int midiChannel, bool down);
    ...
    */
    ...
    //==============================================================================
    /** Receives events from a MyMidiKeyboardState object. */
    ...
    public:
        ...
        //==============================================================================
        /** Called when one of the MyMidiKeyboardState's keys is pressed.

            This will be called synchronously when the state is either processing a
            buffer in its MyMidiKeyboardState::processNextMidiBuffer() method, or
            when a note is being played with its MyMidiKeyboardState::noteOn() method.

            Note that this callback could happen from an audio callback thread, so be
            careful not to block, and avoid any UI activity in the callback.
        */
        virtual void handleNoteOn (MyMidiKeyboardState* source,
                                   int midiChannel, int midiNoteNumber, float velocity) = 0;

        /** Called when one of the MyMidiKeyboardState's keys is released.

            This will be called synchronously when the state is either processing a
            buffer in its MyMidiKeyboardState::processNextMidiBuffer() method, or
            when a note is being played with its MyMidiKeyboardState::noteOff() method.

            Note that this callback could happen from an audio callback thread, so be
            careful not to block, and avoid any UI activity in the callback.
        */
        virtual void handleNoteOff (MyMidiKeyboardState* source,
                                    int midiChannel, int midiNoteNumber, float velocity) = 0;

        virtual void handlePedalDown(MyMidiKeyboardState* source, int midiChannel) = 0;
        virtual void handlePedalUp(MyMidiKeyboardState* source, int midiChannel) = 0;
    };
...
protected:
    ...
    std::bitset<16> keyDown[128];
    std::bitset<16> sustaining[128];
    std::bitset<16> pedalDown;
...
private:
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MyMidiKeyboardState)
};

using MidiKeyboardStateListener = MyMidiKeyboardState::Listener;
